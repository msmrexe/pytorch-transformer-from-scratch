...
class InputEmbeddings(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Multiply by sqrt(d_model) to scale embeddings before adding positional encoding
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float):
        ...
        # Create the division term for sine/cosine
        ...
        # Register as a buffer so it's saved with state_dict but not trained
...
class LayerNormalization(nn.Module):
    def __init__(self, features: int, eps: float = 1e-6):
        super().__init__()
        self.eps = eps
        # Learnable parameters alpha (scale) and bias (shift)
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        if mask is not None:
            # Write a very low value (indicating -inf) to positions where mask == 0
    ...
    def forward(self, q, k, v, mask):
        ...
        # Split into heads by reshaping and transposing
...
class ResidualConnection(nn.Module):
    """
    Implements the Pre-Layer Normalization (Pre-LN) residual connection.
    Pattern: x + dropout(sublayer(norm(x)))
    """
    ...
    def forward(self, x, sublayer):
        # Pre-LN: Normalize BEFORE passing to the sublayer
...
class ProjectionLayer(nn.Module):
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        # NOTE: We use log_softmax here for numerical stability with NLLLoss later.
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int=512, N: int=6, h: int=8, dropout: float=0.1, d_ff: int=2048) -> Transformer:
    """
    Factory function to construct the full Transformer model.
    """
